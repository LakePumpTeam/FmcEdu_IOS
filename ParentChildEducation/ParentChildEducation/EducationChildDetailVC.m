...
@interface EducationChildDetailVC ()<UITableViewDataSource, UITableViewDelegate, NetworkPtc, MWPhotoBrowserDelegate, SDPhotoGroupDelegate>
...
@implementation EducationChildDetailVC
...
- (void)getLikeRequest
{
    ...
    // 刷新页面
    if ([_newsResult.liked boolValue])
    {
        _newsResult.liked = [NSNumber numberWithBool:NO];
        liked = @"false";
    }
    else
    {
        _newsResult.liked = [NSNumber numberWithBool:YES];
        liked = @"true";
    }
    ...
    [parameters setObjectSafe:base64Encode(liked) forKey:@"isLike"];

    // 发送请求
    [NetWorkTask postRequest:kRequestLikeNews
                 forParamDic:parameters
                searchResult:[[BusinessSearchNetResult alloc] init]
                 andDelegate:self forInfo:kRequestLikeNews];
}
...
- (void)getSearchNetBack:(SearchNetResult *)searchResult forInfo:(id)customInfo
{
    ...
    if ([customInfo isEqualToString:kRequestLikeNews])
    {
        [self getSearchNetBackOfLike:searchResult forInfo:customInfo];
    }
    else if ([customInfo isEqualToString:kRequestNewsDetail])
    {
...
- (void)getSearchNetBackOfLike:(SearchNetResult *)searchResult forInfo:(id)customInfo
{
    if (searchResult != nil)
    {
...
- (void)doLikeAction:(UIButton *)sender
{
    // 不可点击，回调中重置可点击
    sender.enabled = NO;
...
- (void)setLikeItem
{
    ...
    if ([_newsResult.liked boolValue])
    {
        [settingButton setImage:[UIImage imageNamed:@"DetailLikeSelect"] forState:UIControlStateNormal];
    }
    else
    {
        [settingButton setImage:[UIImage imageNamed:@"DetailLike"] forState:UIControlStateNormal];

    }

    [settingButton addTarget:self action:@selector(doLikeAction:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)setupRootViewSubs:(UIView *)viewParent
{
    ...
    UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(spaceXStart, spaceYStart, viewParent.width, spaceYEnd-spaceYStart)
...
- (void)setupViewSubsTitleCell:(UIView *)viewParent inSize:(CGSize *)viewSize
{
    ...
    {
        subTitleLabel = [[UILabel alloc] initWithFont:kSmallTitleFont andText:self.vcName andColor:kTextColor withTag:eSubsTitleLabelTag];
        subTitleLabel.backgroundColor = kWhiteColor;

        [viewParent addSubview:subTitleLabel];
    }
    
    CGSize subTitleSize = [self.vcName sizeWithFontCompatible:kSmallTitleFont constrainedToSize:CGSizeMake(viewSize->width-spaceXStart*2, CGFLOAT_MAX) lineBreakMode:NSLineBreakByTruncatingTail];
...
- (void)setupViewSubsContentCell:(UIView *)viewParent inSize:(CGSize *)viewSize
{
    ...
    if (_newsResult.imageUrls && _newsResult.imageUrls.count > 0)
    {
        for (int i = 0; i < _newsResult.imageUrls.count; i++)
        {
            ...
            // 图片
            // =======================================================================
            UIView *imageView = (UIView *)[viewParent viewWithTag:eImageViewTag];
            
            if (imageView == nil)
            {
                imageView = [[UIView alloc] initWithFrame:CGRectZero];
                imageView.tag = eImageViewTag;
