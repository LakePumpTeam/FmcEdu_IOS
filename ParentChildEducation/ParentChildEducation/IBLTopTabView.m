...
#import "IBLTopTabView.h"
#import "IBLButtonForTopbar.h"

@implementation IBLTopTabView
...
- (id)initWithFrame:(CGRect)frame titles:(NSArray *)titles type:(NSString*)type
{
    ...
    if (self)
    {
        for (int i =0 ; i <self.count; i ++)
        {
            ...
            IBLButtonForTopbar *btn ;
            if ([type isEqualToString:@"top" ])
            {
                btn = [[IBLButtonForTopbar alloc] initWithFrameTop:CGRectMake(btnWidth*i, 0, btnWidth, btnHeight)  title:titles[i] ];
             } else
             {
                btn = [[IBLButtonForTopbar alloc] initWithFrame:CGRectMake(btnWidth*i, 0, btnWidth-1, btnHeight)  title:titles[i] ];
             }
...
{
    IBLButtonForTopbar *btnSelected = (IBLButtonForTopbar *)sender;
    
    for (IBLButtonForTopbar *btn in _btnArr)
    {
    ...
    if(_delegate&&[_delegate conformsToProtocol:@protocol(IBLTopTabViewDelegate)])
    {
        [_delegate iBLTopTabSelect:btnSelected tabView:self];
    }
 }
